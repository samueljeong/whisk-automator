...
async function startAutomation() {
  ...
  // MAIN world에 persistent showOpenFilePicker interceptor 주입 (CSP 우회)
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      func: () => {
        if (window.__whiskAutoInterceptorInstalled) {
          console.log('[Whisk Auto MAIN] interceptor 이미 설치됨, 건너뜀');
          return;
        }
        ...
        window.showOpenFilePicker = async function(...args) {
          ...
          if (dataUrl) {
            ...
            console.log('[Whisk Auto MAIN] showOpenFilePicker 가로채기 성공! dataUrl 길이:', dataUrl.length);
            ...
            try {
              ...
              console.log('[Whisk Auto MAIN] 파일 핸들 생성 완료 (' + file.size + ' bytes)');
              return [handle];
            } catch (e) {
              console.error('[Whisk Auto MAIN] 파일 변환 실패:', e);
              document.documentElement.setAttribute('data-whisk-upload-done', 'error');
              return origPicker.apply(this, args);
            }
          }
          // 인터셉트 데이터 없으면 원본 호출
          console.log('[Whisk Auto MAIN] 인터셉트 데이터 없음, 원본 showOpenFilePicker 호출');
          return origPicker.apply(this, args);
        };

        // defineProperty로도 보호 (Whisk가 재할당 시도 대비)
        try {
          var currentFn = window.showOpenFilePicker;
          Object.defineProperty(window, 'showOpenFilePicker', {
            get: function() { return currentFn; },
            set: function(v) {
              console.log('[Whisk Auto MAIN] showOpenFilePicker 재할당 시도 감지, 무시');
            },
            configurable: true
          });
        } catch(e) {
        ...
        console.log('[Whisk Auto MAIN] persistent interceptor 설치 완료');
      }
    });
    console.log('[Popup] MAIN world interceptor 주입 성공');
  } catch (e) {
    console.warn('[Popup] MAIN world interceptor 주입 실패:', e.message);
  }
