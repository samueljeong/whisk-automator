...
function runFlowAutomation(promptsWithCharacters, delayMs, autoDownload, _unused, characters, savePath, scenes, styles, useCustomDir, selectedModel, selectedOutputType) {
  ...
  // 페이지 컨텍스트에서 실행해야 React fiber 접근 가능
  function dumpSlateEditor() {
    var script = document.createElement('script');
    script.textContent = '(' + function() {
      try {
        var el = document.querySelector('[role="textbox"][contenteditable]');
        if (!el) { console.error('[Slate Debug] contenteditable 요소 없음'); return; }

        // React fiber 찾기
        var fiberKey = Object.keys(el).find(function(k) {
          return k.startsWith('__reactFiber$') || k.startsWith('__reactInternalInstance$');
        });
        if (!fiberKey) { console.error('[Slate Debug] React fiber 없음'); return; }

        var fiber = el[fiberKey];
        console.log('[Slate Debug] Fiber 키: ' + fiberKey);

        // fiber tree를 올라가며 Slate editor 찾기
        var current = fiber;
        var editor = null;
        var depth = 0;
        var fiberPath = [];
        ...
        while (current && depth < 50) {
          var name = '(anonymous)';
          if (current.type) {
            name = current.type.displayName || current.type.name || String(current.type).substring(0, 40);
          }

          // editor 찾기: memoizedProps.editor
          if (current.memoizedProps && current.memoizedProps.editor &&
            ...
            fiberPath.push('[' + depth + '] ' + name + ' ★EDITOR FOUND (props.editor)★');
            break;
          }

          // editor 찾기: memoizedState 훅 체인
          if (!editor) {
            ...
            while (hook && hookIdx < 20) {
              ...
              if (val && typeof val === 'object' && !Array.isArray(val) &&
                ...
                editor = val;
                fiberPath.push('[' + depth + '] ' + name + ' ★EDITOR FOUND (hook[' + hookIdx + '])★');
                break;
              }
              // context value 체크
              if (hook.queue && hook.queue.lastRenderedState &&
                  typeof hook.queue.lastRenderedState === 'object') {
                var ctx = hook.queue.lastRenderedState;
                if (ctx && ctx.apply && Array.isArray(ctx.children)) {
                  editor = ctx;
                  fiberPath.push('[' + depth + '] ' + name + ' ★EDITOR FOUND (context)★');
                  break;
                }
              }
              hook = hook.next;
          ...
          // 유용한 컴포넌트 이름만 기록
          if (name !== '(anonymous)') {
            fiberPath.push('[' + depth + '] ' + name);
          }
          current = current.return;
          depth++;
        }

        console.log('[Slate Debug] Fiber 경로:\n' + fiberPath.join('\n'));

        if (!editor) {
          console.error('[Slate Debug] Slate editor 인스턴스를 찾을 수 없음. 전체 fiber 덤프:');
          current = fiber;
          depth = 0;
          while (current && depth < 30) {
            var n = current.type ? (current.type.displayName || current.type.name || current.type) : null;
            var propsKeys = current.memoizedProps ? Object.keys(current.memoizedProps).join(',') : '';
            console.log('  [' + depth + '] type=' + n + ' props=[' + propsKeys + ']');
            current = current.return;
            depth++;
          }
          return;
        ...
        // editor 정보 덤프
        console.log('[Slate Debug] ===== EDITOR 정보 =====');
        console.log('[Slate Debug] editor 키:', Object.keys(editor).join(', '));
        console.log('[Slate Debug] editor.children 수:', editor.children.length);
        console.log('[Slate Debug] editor.selection:', JSON.stringify(editor.selection));
        console.log('[Slate Debug] editor.children 전체:');
        ...
        function findVoids(nodes, path) {
          for (var i = 0; i < nodes.length; i++) {
            ...
            // void 노드 특성: isVoid, type에 특정 키워드, children이 빈 text
            if (node.type && node.type !== 'paragraph' && node.type !== 'text') {
              voidNodes.push({ path: curPath, type: node.type, keys: Object.keys(node), node: node });
            }
            if (node.children) findVoids(node.children, curPath);
          }
        }
        findVoids(editor.children, []);

        if (voidNodes.length > 0) {
          console.log('[Slate Debug] ===== VOID/특수 노드 (' + voidNodes.length + '개) =====');
          voidNodes.forEach(function(v) {
            console.log('[Slate Debug] path=' + JSON.stringify(v.path) + ' type="' + v.type + '" keys=[' + v.keys.join(',') + ']');
            console.log(JSON.stringify(v.node, null, 2));
          });
        } else {
          console.log('[Slate Debug] void/특수 노드 없음 (레퍼런스가 삽입된 상태에서 다시 실행해주세요)');
        }

        // editor를 window에 저장 (추후 삽입 테스트용)
        window.__slateEditor = editor;
        console.log('[Slate Debug] window.__slateEditor에 저장됨 (콘솔에서 직접 조작 가능)');

      } catch(e) {
        console.error('[Slate Debug] 오류:', e.message, e.stack);
      }
    } + ')()';
